...
@Component({
  selector: 'new-financial-reports',
  templateUrl: './new-financial-reports.component.html',
  styleUrls: ['./new-financial-reports.component.sass']
})
export class NewFinancialReportsComponent implements OnInit {
  nuevo: {
    title: {
      es: string,
      en: string
          ...
          if (this.nuevo.description.en.length < 4 || this.nuevo.description.en.length > 2000) {
            ...
            if (!(this.nuevo.file.es.size > 0 && this.nuevo.file.en.size > 0)) {
              ...
              if (!(this.nuevo.file.es.size < 51200000 && this.nuevo.file.en.size < 51200000)) {
                ...
                this.http.post(this.general.api + "admin/financialReports", formData, this.auth.options).subscribe(resp => {
                  this.general.loaderHidden();
                  this.general.dialog!.show("", this.general.getText({ es: "Reporte financiero guardada exitosamente", en: "Financial report successfully saved" }), this.general.getText({ es: "Aceptar", en: "Ok" }), undefined, () => { })
                  this.back();
                }, error => {
                  this.general.loaderHidden();
                });
