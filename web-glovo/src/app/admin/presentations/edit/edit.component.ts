...
export class EditPresentationsComponent implements OnInit {
  ...
  show() {
    ...
    if (options.headers["Content-Language"] == "es") {
      ...
      this.http.get(this.general.api + "admin/presentations/" + this.data.id, options).subscribe((data: any) => {
  ...
  downEn() {
    ...
    this.general.downCode.show((pass: string) => {
      this.http.post(this.general.api + "admin/presentations/" + this.data.id + "/download", { password: pass }, { headers: options.headers, responseType: 'blob' as 'json' }).subscribe((resp: any) => {
