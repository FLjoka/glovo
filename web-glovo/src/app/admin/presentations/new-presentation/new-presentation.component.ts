...
@Component({
  selector: 'new-presentation',
  templateUrl: './new-presentation.component.html',
  styleUrls: ['./new-presentation.component.sass']
})
export class NewPresentationComponent implements OnInit {
          ...
          if (this.nuevo.description.en.length < 4 || this.nuevo.description.en.length > 2000) {
            ...
            if (!(this.nuevo.file.es.size > 0 && this.nuevo.file.en.size > 0)) {
              ...
              if (!(this.nuevo.file.es.size < 51200000 && this.nuevo.file.en.size < 51200000)) {
                ...
                this.http.post(this.general.api + "admin/presentations", formData, this.auth.options).subscribe(resp => {
                  this.general.loaderHidden();
                  this.general.dialog!.show("", this.general.getText({ es: "Presentación guardada exitosamente", en: "Presentation saved successfully" }), this.general.getText({ es: "Aceptar", en: "Ok" }), undefined, () => { })
                  this.back();
                }, error => {
                  this.general.loaderHidden();
                });
  ...
  elegirEn() {
    (document.getElementById("presentationIngles") as HTMLInputElement).click();
  }
  elegir() {
    (document.getElementById("presentationEspañol") as HTMLInputElement).click();
  }
}
