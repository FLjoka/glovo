...
export class DetailComponent implements OnInit {
  ...
  enviar() {
    this.enviando = true;
    this.http.post(this.general.api + "whistleblowerRequests/" + this.data.id + "/resend", {}, this.auth.options).subscribe(resp => {
      this.enviando = false;
    }, error => {
      this.enviando = false;
  ...
  download() {
    ...
    this.general.downCode.show((pass: string) => {
      ...
      this.http.post(this.general.api + "whistleblowerRequests/" + this.data.id + "/download", { password: pass }, { headers: options.headers, responseType: 'blob' as 'json' }).subscribe((resp: any) => {
  ...
  show() {
    this.load = true;
    this.http.get(this.general.api + "whistleblowerRequests/" + this.data.id, this.auth.options).subscribe(resp => {
      this.allData = resp;
      this.load = false;
    }, error => {
      this.load = false;
