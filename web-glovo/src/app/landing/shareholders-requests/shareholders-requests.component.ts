...
export class ShareholdersRequestsComponent implements OnInit {
  ...
  constructor(public individual : IndividualService, private http : HttpClient, private auth : AuthService, public general: GeneralService,private route : Router, private fb: UntypedFormBuilder, private messageService: MessageService) {
  ...
  ngOnInit() {
    window.scrollTo(0, 0)
    this.options = [];
    this.individual.requestsOptions.forEach(  ( option : any  )=> {
  ...
  ngOnDestroy(){
    this.lenguageSubscriber.unsubscribe();
  }
  addSingle() {
    this.messageService.add({ severity: 'success', summary: 'formulario', detail: 'Formulario enviado' });
  }
  addSingleError() {
    this.messageService.add({ severity: 'error', summary: 'formulario', detail: 'Error al Enviar el formulario' });
  }
  elegir(){
  ...
  submit() {
    ...
    try {
     ...
     this.http.post(this.general.api + "shareholderRequests" , formData, this.auth.options).subscribe ( resp => {
      // this.addSingle();
      this.docName = "";
      this.form.reset();
      ...
      if (invalido.hasError("required"))
        this.general.dialog?.show("", this.general.getText({ es: "Por favor complete el campo \"" + this.getControlsText(e.message) + "\"", en: "Please fill in the \"" + this.getControlsText(e.message) + "\" field" }), this.general.getText({ en: "Accept", es: "Aceptar" }),
          undefined, () => { }, () => { });
    }
